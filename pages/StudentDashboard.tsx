...
import { 
  ...
  Target, Circle, Search, Star, Award, Zap
} from 'lucide-react';
        ...
        <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-[32px] flex flex-col md:flex-row justify-between items-center gap-8 shadow-sm overflow-hidden relative group">
           ...
           <div className="relative z-10 space-y-3">
              <div className="flex items-center gap-3">
                 <div className={`px-3 py-1 ${levelData.bg} ${levelData.color} rounded-lg text-[9px] font-black tracking-widest uppercase shadow-sm border border-current/10`}>
                    LEVEL {levelData.level}
                 </div>
                 <span className="text-[12px] font-bold text-zinc-400 tracking-tight">#{userRank} GLOBAL RANK</span>
              </div>
              <h1 className="text-3xl font-black tracking-tight leading-tight text-zinc-900 dark:text-white flex items-center gap-3">
                Hello, {user.name.split(' ')[0]}
                {progress.currentStreak >= 3 && <Flame className="text-orange-500 fill-orange-500 animate-pulse" size={24} />}
              </h1>
              <p className="text-zinc-400 font-semibold text-[14px]">{levelData.badge}</p>
           </div>

           <div className="relative z-10 flex gap-12">
              <div className="text-center group/xp">
                 <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-2">SYNC XP</p>
                 <div className="text-3xl font-black text-zinc-900 dark:text-white group-hover:scale-110 transition-transform cursor-default">{progress.points.toLocaleString()}</div>
              </div>
              <div className="text-center group/streak">
                 <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-2">STREAK</p>
                 <div className="text-3xl font-black flex items-center gap-2.5 justify-center text-zinc-900 dark:text-white group-hover:scale-110 transition-transform cursor-default">
                   <Flame className="text-orange-500 fill-orange-500" size={24} /> {progress.currentStreak}
                 </div>
              </div>
           </div>
        </section>
                ...
                {streakDots.map((dot, idx) => (
                  <div 
                    ...
                    className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                      dot.completed ? 'bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.4)] scale-110' : 
                      dot.isToday ? 'border-2 border-zinc-300 dark:border-zinc-700 animate-pulse' : 'bg-zinc-100 dark:bg-zinc-800'
                    }`}
                  />
                ))}
              </div>
